...
function getSidebarWidth() {
  const sidebarWString = getComputedStyle(document.documentElement).getPropertyValue('--sidebar-w').trim();
  return parseInt(sidebarWString, 10) || 0;
}

export const UIManager = {
  init(threeRenderer, threeCamera, canvasElement, rotationControllerInstance) { // Added rotationControllerInstance
  ...
  setupEventHandlers() {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && cartPane && cartPane.classList.contains('open')) {
        this.closeCart();
      }
    });
    document.addEventListener('click', (e) => {
      if (cartPane && cartPane.classList.contains('open') && 
          !cartPane.contains(e.target) && e.target !== cartBtn) {
        this.closeCart();
      }
    });
    ...
    if (checkoutBtn) checkoutBtn.onclick = () => alert("Grazie! Funzione checkout da implementare.");
    ...
    if (autoRotateButton && rotationControllerRef) {
      ...
      console.warn("RotationController instance not provided to UIManager for auto-rotate button.");
    }
  },
  ...
  handleResize() {
    ...
    console.log(`UIManager.handleResize: sidebarOpen=${sidebarOpen}, sidebarActualWidth=${sidebarWidth}, newCanvasWidth=${newWidth}, newCanvasHeight=${newHeight}`);
    ...
    if (rendererRef && cameraRef) {
      ...
      console.warn("Renderer or Camera not available in UIManager during handleResize. Dimensions logged only.");
    }
  },
  ...
  populateMenuItems(data) {
    ...
    if (bestsellerList) {
      bestsellerList.innerHTML = data.filter(i => i.categoria === 'bestseller').map(renderCard).join('');
    }
    if (primiList) {
      primiList.innerHTML = data.filter(i => i.categoria === 'primo').map(renderCard).join('');
    }
    if (secondiList) {
      secondiList.innerHTML = data.filter(i => i.categoria === 'secondo').map(renderCard).join('');
    }
  ...
  updateCartUI(anim = false) {
    ...
    if (anim && badge) {
      badge.animate([
        { transform: 'scale(.8)' }, { transform: 'scale(1.3)' }, { transform: 'scale(1)' }
      ], { duration: 350, easing: 'ease-out' });
    }
    if (cartItemsList) {
      cartItemsList.innerHTML = this.cart.map(i => `<li>${i.name} – €${i.price.toFixed(2)}</li>`).join('');
    }
    if (cartTotalEl) {
      cartTotalEl.textContent = 'Totale €' + this.cart.reduce((s, i) => s + i.price, 0).toFixed(2);
    }
...
const menuData = [
  { name: "Bruschetta", file: "Bruschetta.glb", price: 7.5, desc: "Pane croccante con pomodorini, basilico e olio EVO.", img: "./asset/bruschetta.jpg", categoria: "bestseller" },
  { name: "Pizza Margherita", file: "Margherita.glb", price: 8.5, desc: "Classica con mozzarella e basilico.", img: "./asset/margherita.jpg", categoria: "bestseller" },
  { name: "Spaghetti Carbonara", file: "Carbonara.glb", price: 11.0, desc: "Guanciale, pecorino e uovo.", img: "./asset/carbonara.jpg", categoria: "primo" },
  { name: "Lasagne", file: "Lasagne.glb", price: 12.0, desc: "Ragù, besciamella e parmigiano.", img: "./asset/lasagne.jpg", categoria: "primo" },
  { name: "Tagliata di Manzo", file: "Tagliata.glb", price: 17.0, desc: "Controfiletto con rucola e grana.", img: "./asset/tagliata.jpg", categoria: "secondo" },
  { name: "Salmone alla griglia", file: "Salmone.glb", price: 15.0, desc: "Filetto di salmone, salsa agrumi.", img: "./asset/salmone.jpg", categoria: "secondo" },
];
