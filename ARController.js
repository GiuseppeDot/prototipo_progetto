...
export const ARController = {
  init(threeScene, threeCamera, threeRenderer) {
    ...
    arToolkitSource = new THREEx.ArToolkitSource({
      sourceType: 'webcam',
      // Optional: add sourceWidth and sourceHeight if specific resolutions are needed, but defaults are usually fine.
      // sourceWidth: 640,
      // sourceHeight: 480,
    });
    console.log("ARController: ArToolkitSource created.");

    arToolkitSource.init(() => {
      console.log("ARController: ArToolkitSource initialized successfully. Video DOM element:", arToolkitSource.domElement);
      // Handle resize after source is ready
      setTimeout(() => {
        this.onResize();
        console.log("ARController: onResize called after ArToolkitSource init.");
      }, 500);
    }, (error) => {
      console.error("ARController: ArToolkitSource failed to initialize!", error);
      // Display a user-friendly message or alert if possible.
      // e.g., UIManager.showError("Could not access camera. Please ensure permissions are granted and no other application is using it. Error: " + error);
      alert("Error initializing camera. Please check permissions. Error: " + error); // Simple alert for now
    });

    // Initialize AR Toolkit Context (Marker Detection Engine)
    arToolkitContext = new THREEx.ArToolkitContext({
      cameraParametersUrl: THREEx.ArToolkitContext.baseURL + '../data/data/camera_para.dat', // Default camera parameters
    ...
    arToolkitContext.init(() => {
      ...
      console.log("ARController: AR Toolkit Context initialized, camera projection matrix set.");
    });
    ...
    console.log("ARController initialized with THREEx AR.js components. Placeholder model added.");
  },
  ...
  onResize() {
    ...
    if (arToolkitSource && arToolkitSource.ready) {
      ...
      if (arToolkitContext && arToolkitContext.arController) {
        ...
        camera.projectionMatrix.copy(arToolkitContext.getProjectionMatrix());
        console.log("ARController.onResize: AR components and camera projection updated based on video dimensions.");
      }
    }
  },
  ...
  update() { 
    ...
    if (markerRoot && camera && placeholderModel) { // Ensure all are initialized
        if (markerRoot.visible) { 
            if (!modelHasBeenPlaced) {
                ...
                console.log("ARController: Model placed at screen center.", placeholderModel.position);
            }
        } else {
