...
function onMouseDown(event) {
  ...
  if (isAutoRotating) {
    ...
    console.log("RotationController: Auto-rotate stopped by manual interaction.");
  }
...
function onMouseMove(event) {
  ...
  const currentMouseX = event.clientX || (event.touches && event.touches[0].clientX);
...
export const RotationController = {
  ...
  toggleAutoRotate() {
    ...
    if (isAutoRotating && rotatableModel) {
      if (isRotating) { // If manual rotation was active
  ...
  update(deltaTime) {
    if (isAutoRotating && rotatableModel && !isRotating) { // Only auto-rotate if not manually rotating
